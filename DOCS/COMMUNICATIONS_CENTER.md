...
Каналы WhatsApp/Instagram подключаются адаптерами в PR-5…6.

## Входящие webhooks

`POST /communications/webhooks/telegram` — приём входящих Telegram (официальный
Bot API webhook), без авторизации пользователя. Подлинность — по секретному
токену вебхука (заголовок `X-Telegram-Bot-Api-Secret-Token` = `TELEGRAM_WEBHOOK_SECRET`):
секрет не задан → приём отключён (503); неверный секрет → 403. Создаёт входящее
сообщение (`record_incoming`). Организация определяется по telegram-коннектору.
