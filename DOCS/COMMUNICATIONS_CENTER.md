...
Модель `broadcasts` (миграция 0044) + сервис `app/services/broadcasts.py`. Рассылка
формирует по одному `CommunicationMessage` на контакт (`broadcast_id`) и управляет
группой: цели, тестовая отправка (sandbox, не затрагивает целевых), предпросмотр,
согласование (SoD для внешних), планирование, отправка, **отчёт о доставке**
(агрегат по статусам), **повтор только неуспешным**.

Защита: исключаются получатели без согласия, в стоп-листе и **отписавшиеся**
(`communication_contacts.unsubscribed`); анти-спам — один контакт получает не более
одного сообщения в рассылке; внешний канал с шаблоном требует утверждённого шаблона
(WhatsApp). Реальная отправка по умолчанию выключена (sandbox).

API `/communications/broadcasts` (список/создание/детали/targets/preview/test/
submit-approval/approve/send/retry/report); RBAC `communication.manage` (создание/
цели/тест), `communication.approve` (согласование), `communication.send` (отправка/
повтор), `communication.view` (чтение). Экран «Коммуникации» → вкладка «Рассылки».
