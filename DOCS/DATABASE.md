...
- `materials` — приобретённые материалы;
- `warehouse_movements` — оприходование приобретённых материалов;
- `equipment` и `vehicles` — расходы на технику и транспорт;
...
В `audit_log` обязательно фиксируются:
